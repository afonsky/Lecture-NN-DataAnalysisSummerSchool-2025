...
**Батч**-подход тесно связан с градиентными методами. Батчи (пакеты данных) позволяют производить параллельное обучение DL сетей (то есть **значительно** сократить время обучения).
...
* **Большие** батчи дают более точную оценку градиента
* Количество батчей стоит выбирать исходя из размера доступной видеопамяти
* **Малые** батчи могут обеспечить регуляризирующий эффект

Также очень важно, чтобы минибатчи были выбраны случайным образом, иначе будет переобучение к последовательности данных

Реализовать батчи в PyTorch можно при помощи [DataLoader](https://pytorch.org/docs/stable/data.html#torch.utils.data.DataLoader)

**Нормализация батча** - это шаг гиперпараметра $\gamma, \beta$, который нормализует батч $\{x_i\}$. Если $\mu_B, \sigma_B^2$ это среднее и дисперсия данных в батче, то нормализация делается следующим образом:
